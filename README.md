...
| **Capital Growth 376 Days** | Накопление BTC (реальный медвежий рынок) | Симуляция реального медвежьего рынка 2021-2022 (10.11.2021 — 21.11.2022) |
...
| Capital Growth 376 Days | [capital_growth_376days.html](https://cgs-simulation.vercel.app/capital_growth_376days.html) |
...
На странице симуляции отображается таблица исторических падений Bitcoin, показывающая, что BTC становится более **зрелым** активом:
...
💡 **Вывод:** С каждым циклом падения становятся менее глубокими, но для стресс-тестирования стратегии мы симулируем серьёзное падение -76%, соответствующее тяжёлому медвежьему рынку, близкому к циклу 2021-2022.
...
### Capital Growth 376 Days (Real Bear Market 2021-2022)

Симуляция с использованием **исторических данных** последнего медвежьего рынка Bitcoin.

**Ключевые параметры:**
- Период: **10 ноября 2021 — 21 ноября 2022** (376 дней)
- Начальная цена (ATH): **$68,982**
- Конечная цена (минимум): **$15,768**
- Ежедневное падение: **-0.45%** (компаунд)
- Итоговое падение: **-77.2%**
- Триггеры: HF < 1.45 (идентичны Dynamic версии)

**Контекст: Реальный медвежий рынок 2021-2022**

Симуляция воспроизводит реальные события:
- **10 ноября 2021** — Bitcoin достигает All-Time High ($68,982)
- **2022 Q1-Q3** — Постепенное падение на фоне повышения ставок ФРС
- **Май 2022** — Крах Terra Luna усиливает давление на рынок
- **11 ноября 2022** — Крах FTX становится катализатором финального падения
- **21 ноября 2022** — Bitcoin достигает минимума ($15,768)
...
End Price: $15,768
Duration: 376 days
Daily Multiplier = (15768 / 68982)^(1/376) = 0.9954967
Daily Drop: -0.45%

Verification: 68982 × 0.9954967^376 ≈ 15768 ✓
```

**Особенности v1.0 (с DCA):**
- Регулярные DCA-инвестиции (по умолчанию $1000 каждые 30 дней)
- Автоматическое распределение DCA по зонам портфеля
- Балансировка S/G и Y/L Ratio через DCA
- Реальные даты отображаются в таблице (формат DD.MM)
- Колонка "Дата" для исторического контекста
- Графики фильтруются (каждый 15-й день + триггеры)
- Начальная цена зафиксирована ($68,982, readonly)
...
**Логика распределения DCA (идентична 270 Days):**
1. **S/G < target**: DCA → Stability Zone (перекос в Growth)
2. **S/G >= target, Y/L < 1.0**: DCA → GM Pool (Yield Zone мала)
3. **S/G >= target, Y/L >= 1.0**: DCA → BTC в залог (всё в норме)
...
- **Проверка надежности** — тестирует стратегию на реальных событиях (крах FTX, Terra Luna)
- **Длительность** — полный цикл медвежьего рынка (~12.5 месяцев)
- **Референс** — можно сравнить результаты с синтетической симуляцией 270 дней
